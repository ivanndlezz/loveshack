...
class DateTimePicker {
  constructor(container, options = {}) {
    ...
    this.yearRange = { start: Math.floor(this.currentYear/12)*12, end: Math.floor(this.currentYear/12)*12+11 };
    ...
    this.MONTHS_SHORT = ['ENE','FEB','MAR','ABR','MAY','JUN','JUL','AGO','SEP','OCT','NOV','DIC'];
    this.DAYS_ES = ['Lu','Ma','Mi','Ju','Vi','Sá','Do'];

    this.DISABLED_RULES = {
      allow_past_days: true, allow_past_months: true, allow_past_years: true,
      allow_weekends: true, allow_same_day: true, allow_future_limit: null
    };
    ...
    if (options.initialFrom) this._parseTime(options.initialFrom, this.slots.from);
  ...
  _id(suffix) { return this._uid + '-' + suffix; }
  ...
  render() {
    ...
    const titleText = selDate ? `${selDate.getDate()} de ${this.MONTHS_ES[selDate.getMonth()]}` : '';
    const fromLabel = this.slots.from.confirmed ? this._fmtSlot(this.slots.from) : '';
  ...
  _bindEvents() {
    ...
    this._el('cal-prev').onclick = () => this.viewMode==='year'?this._changeYearRange(-1):this._changeMonth(-1);
    this._el('cal-next').onclick = () => this.viewMode==='year'?this._changeYearRange(1):this._changeMonth(1);
    ...
    this._el('btn-ok').onclick = () => { this._confirmTime(); this._closeSheet(); };
    this._el('opt-am').onclick = () => this._setAMPM('AM');
    this._el('opt-pm').onclick = () => this._setAMPM('PM');

    const inpH = this._el('inp-h'), inpM = this._el('inp-m');
  ...
  _renderCalendarView() {
    ...
    setTimeout(() => {
      ...
      else if (this.viewMode === 'month') content.innerHTML = this._renderMonthView();
  ...
  _renderDayView() {
    ...
    return `<div class="dtp-cal-grid">${this.DAYS_ES.map(d=>`<div class="dtp-cal-dow">${d}</div>`).join('')}${days.map(({day,cur,date})=>{
      ...
      const ok = (this.DISABLED_RULES.allow_past_days||date>=this.today) && cur;
      const dis = !ok;
  ...
  _renderMonthView() {
    ...
    return `<div class="dtp-month-view">${this.MONTHS_ES.map((n,i)=>{
      const isCur = i===new Date().getMonth()&&this.currentYear===new Date().getFullYear();
      const ok = this.DISABLED_RULES.allow_past_months || (this.currentYear>new Date().getFullYear()) || (this.currentYear===new Date().getFullYear()&&i>=new Date().getMonth());
      return `<button class="dtp-month-button ${isCur?'current-month':''} ${!ok?'disabled':''}" ${!ok?'disabled':''}>${n}</button>`;
    }).join('')}</div>`;
  }

  _renderYearView() {
    this._el('cal-label').textContent = `${this.yearRange.start} – ${this.yearRange.end}`;
    return `<div class="dtp-year-view">${Array.from({length:this.yearRange.end-this.yearRange.start+1},(_,i)=>this.yearRange.start+i).map(yr=>{
      const isCur = yr===new Date().getFullYear();
      const ok = this.DISABLED_RULES.allow_past_years || yr>=new Date().getFullYear();
      return `<button class="dtp-year-button ${isCur?'current-year':''} ${!ok?'disabled':''}" ${!ok?'disabled':''}>${yr}</button>`;
    }).join('')}</div>`;
  }

  _attachCalendarListeners() {
    this._el('cal-content').querySelectorAll('.dtp-cal-cell[data-ts]').forEach(c=>{
    ...
    this._el('cal-content').querySelectorAll('.dtp-month-button:not(.disabled)').forEach((b,i)=>{
      b.addEventListener('click',()=>{ this.currentMonth=new Date(this.currentYear,i,1); this.viewMode='day'; this._renderCalendarView(); });
    });
    this._el('cal-content').querySelectorAll('.dtp-year-button:not(.disabled)').forEach(b=>{
      b.addEventListener('click',()=>{ this.currentYear=parseInt(b.textContent); this.viewMode='month'; this._renderCalendarView(); });
    });
  }

  _onDateSelected() {
    this._renderCalendarView();
    const m = this.selectedDate.getMonth(), d = this.selectedDate.getDate();
  ...
  _changeMonth(delta) {
    if (this.viewMode==='day') this.currentMonth = new Date(this.currentMonth.getFullYear(), this.currentMonth.getMonth()+delta, 1);
    else if (this.viewMode==='month') this.currentYear += delta;
    this._renderCalendarView();
  }
  _changeYearRange(delta) { this.yearRange.start+=delta*12; this.yearRange.end+=delta*12; this._renderCalendarView(); }
  _toggleViewMode() { this.viewMode = this.viewMode==='day'?'month':this.viewMode==='month'?'year':'day'; this._renderCalendarView(); }

  // ─── TIME WHEELS ───
  _initWheels() {
    const tH = this._el('trk-h'), tM = this._el('trk-m');
    tH.innerHTML = this.hours.map(h=>`<div class="dtp-wheel-item" ${h<10?'data-dtp-number="left-zero"':''}>${h}</div>`).join('');
    tM.innerHTML = this.minutes.map(m=>`<div class="dtp-wheel-item" ${m<10?'data-dtp-number="left-zero"':''}>${m}</div>`).join('');
    tH.addEventListener('scroll',()=>this._handleScroll('h',tH,this.hours));
    tM.addEventListener('scroll',()=>this._handleScroll('m',tM,this.minutes));
  }
  ...
  _highlightActive(track, idx) {
    track.querySelectorAll('.dtp-wheel-item').forEach((it,i)=>it.classList.toggle('active',i===idx));
  }

  _getAmpm(h) { return (h>=1&&h<=6)||h===12?'PM':'AM'; }
  ...
  _syncFromInputs(e) {
    ...
    if (raw.length>1&&raw.startsWith('0')) { raw=parseInt(raw,10).toString(); e.target.value=raw; }
    let vH = parseInt(this._el('inp-h').value), vM = parseInt(this._el('inp-m').value);
    if (e.target===this._el('inp-h') && this._el('inp-h').value.length>=2) this._el('inp-m').focus();
    if (!isNaN(vH)) { this.slots[this.activeSlot].h=Math.max(0,Math.min(12,vH)); this._scrollToValue('h',this.slots[this.activeSlot].h); }
    if (!isNaN(vM)) { this.slots[this.activeSlot].m=Math.max(0,Math.min(59,vM)); this._scrollToValue('m',this.slots[this.activeSlot].m); }
  }

  _handleKeyDown(e) {
    if (e.key.toLowerCase()==='a') { e.preventDefault(); this._setAMPM('AM'); return; }
    if (e.key.toLowerCase()==='p') { e.preventDefault(); this._setAMPM('PM'); return; }
    if (e.key==='Enter') {
      if (e.target===this._el('inp-h')) this._el('inp-m').focus();
      else { this._confirmTime(); this._closeSheet(); }
    }
  }
  ...
  _scrollToValue(type, value) {
    ...
    if (idx!==-1) track.scrollTo({top:idx*this.STEP, behavior:'smooth'});
  }
  ...
  _openSheet(slot) {
    ...
    this._el('t-from')[slot==='from'?'setAttribute':'removeAttribute']('data-state','active');
    this._el('t-to')[slot==='to'?'setAttribute':'removeAttribute']('data-state','active');
  ...
  _syncSummary() {
    ...
    if (!this.selectedDate) { sub.textContent=''; return; }
    if (!this.slots.from.confirmed) sub.textContent='Elige la hora para completar';
    else if (!this.slots.to.confirmed) sub.textContent=`Desde ${this._fmtSlot(this.slots.from)} · elige hora fin`;
    else sub.textContent = `${this._fmtSlot(this.slots.from)} → ${this._fmtSlot(this.slots.to)}`;
  }

  _updateDuration() {
    const row = this._el('dur-row'), txt = this._el('dur-text');
    if (this.slots.from.confirmed && this.slots.to.confirmed) {
  ...
  getValue() {
    ...
    return { tripDate, startTime: fmt24(this.slots.from), endTime: fmt24(this.slots.to) };
  }
